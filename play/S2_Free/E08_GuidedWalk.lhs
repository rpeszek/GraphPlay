2.08 Free Polymorphism.  Guided Walk polyglot DSL.  Effectful fold interpreters.
------
This example combines ChoiceDSL (from 2.07) with VWalkDSL (implemented in this library) to create a polyglot guided walk. 
...
so, the polyglot DSL can be interpreted with the same interpretInM method (see 2.07).
\begin{code}
...
The 'hardcoded' type for the polyglot DSL is (see 2.06):
\begin{code}
type GuidedWalkDSL v  = Free ((VWalkInstructions v) :+: (ChoiceInstructions v))
\end{code}

I start with 'atomic' program which asks VWalk DSL what are the neighbors, asks 
choice DSL which of them to pick and walks to that neighbor.  I could define it like so:
\begin{code}
guidedStep' ::  forall v.  GuidedWalkDSL v v
guidedStep'  = undefined
\end{code}

However this would hardcode DSL and would lock me from using this instruction in even higher
...
\begin{code}
simpleWalk :: forall v f. ((VWalkInstructions v) :<: f, (ChoiceInstructions v) :<: f) => 
                     Int -> Free f [v]
simpleWalk steps = do
...
testWalk :: IO [Int]
testWalk = runGuidedWalk (simpleWalk 3) testGraph 0 
\end{code}

*Limitations, notes, not likes*
 - notice ugly type specification '(:: Free f)' inside the implementation of simpleWalk. GHC errors out without it. 
   Interestingly, there would be no need for that type hint if I have typed both guidedStep and simpleWalk
...
*Remaining questions and TODOs*
 - The approach in which language instruction type (e.g. ChoiceInstructions) is used to define interpreter type class instance 
   ...
   This is not optimal and I need to figure out a clean way to specify other interpreters (for example, random choice in Choice DSL).
 - Folding. I need to finally digest "Bananas" paper. Recursion schemes seem to be very relevant to this pattern.
 - Injections vs bijections.  DSLs are embedded one into the other using type class (:<:) which requires that there is an injection
   ...
   It would be cool to figure out how this bijection can be used when interpreting.  It seems very logical that it can be used.
